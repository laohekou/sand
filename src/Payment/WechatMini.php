...
class WechatMini extends AbstractGateway
{
    ...
    public function orderCreate(array $body)
    {
        ...
        if( isset($result['sign']) && isset($result['data']) ) {
            ...
            if(! $this->app->decrypt->verify($result['data'], $result['sign']) ) {
                throw new UnauthorizedException('orderPay 小程序验证签名失败');
            }
        }else{
            throw new UnauthorizedException('orderPay 小程序杉德数据失败');
        }
    ...
    public function orderRefund(array $body)
    {
        ...
        if( isset($result['sign']) && isset($result['data']) ) {
            ...
            if(! $this->app->decrypt->verify($result['data'], $result['sign']) ) {
                throw new UnauthorizedException('orderRefund 小程序验证签名失败');
            }
        }else{
            throw new UnauthorizedException('orderRefund 小程序杉德数据失败');
        }
    ...
    public function orderQuery(array $body)
    {
        ...
        if( isset($result['sign']) && isset($result['data']) ) {
            ...
            if(! $this->app->decrypt->verify($result['data'], $result['sign']) ) {
                throw new UnauthorizedException('orderQuery 小程序验证签名失败');
            }
        }else{
            throw new UnauthorizedException('orderQuery 小程序杉德数据失败');
        }
    ...
    public function clearfileDownload(array $body)
    {
        ...
        if( isset($result['sign']) && isset($result['data']) ) {
            ...
            if(! $this->app->decrypt->verify($result['data'], $result['sign']) ) {
                throw new UnauthorizedException('clearfileDownload 小程序验证签名失败');
            }
        }else{
            throw new UnauthorizedException('clearfileDownload 小程序杉德数据失败');
        }
