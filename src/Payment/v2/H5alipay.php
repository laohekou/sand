        ...
        try {
            ...
            if( isset($result['sign']) && isset($result['data']) ) {
                ...
                if(! $this->verify($result['data'], $result['sign']) ) {
                    throw new BusinessException('orderRefund H5alipay验证签名失败', $this);
                }
            }else{
                throw new BusinessException('orderRefund H5alipay杉德数据失败', $this);
            }
    ...
    public function orderQuery(array $body)
    {
        ...
        try {
            ...
            if( isset($result['sign']) && isset($result['data']) ) {
                ...
                if(! $this->verify($result['data'], $result['sign']) ) {
                    throw new BusinessException('orderQuery H5alipay验证签名失败', $this);
                }
            }else{
                throw new BusinessException('orderQuery H5alipay杉德数据失败', $this);
            }
            return json_decode($result['data'],true);
        }catch (\Throwable $e) {
            $newException = $e instanceof SandException ? $e : new BusinessException(
    ...
    public function orderConfirmPay(array $body)
    {
        ...
        try {
            ...
            if( isset($result['sign']) && isset($result['data']) ) {
                ...
                if(! $this->verify($result['data'], $result['sign']) ) {
                    throw new BusinessException('orderConfirmPay H5alipay验证签名失败', $this);
                }
            }else{
                throw new BusinessException('orderConfirmPay H5alipay杉德数据失败', $this);
            }
            return json_decode($result['data'],true);
        }catch (\Throwable $e) {
            $newException = $e instanceof SandException ? $e : new BusinessException(
    ...
    public function orderMcAutoNotice(array $body)
    {
        ...
        try {
            ...
            if( isset($result['sign']) && isset($result['data']) ) {
                ...
                if(! $this->verify($result['data'], $result['sign']) ) {
                    throw new BusinessException('orderMcAutoNotice H5alipay验证签名失败', $this);
                }
            }else{
                throw new BusinessException('orderMcAutoNotice H5alipay杉德数据失败', $this);
            }
            return json_decode($result['data'],true);
        }catch (\Throwable $e) {
            $newException = $e instanceof SandException ? $e : new BusinessException(
    ...
    public function clearfileDownload(array $body)
    {
        ...
        try {
            ...
            if( isset($result['sign']) && isset($result['data']) ) {
                ...
                if(! $this->verify($result['data'], $result['sign']) ) {
                    throw new BusinessException('clearfileDownload H5alipay验证签名失败', $this);
                }
            }else{
                throw new BusinessException('clearfileDownload H5alipay杉德数据失败', $this);
            }
            return json_decode($result['data'],true);
        }catch (\Throwable $e) {
            $newException = $e instanceof SandException ? $e : new BusinessException(
